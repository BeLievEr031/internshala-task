...
class ToDoController {
    ...
    async edit(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (value.action === "edit") {
                ...
                const isExists = await TodoModel.findOne({ title: value.title })
                // console.log(isExists?._id, query.id)
                // console.log(JSON.stringify(isExists?._id) !== query.id)
                // if (isExists ) {
                //     return next(createError(409, "Title Already Exists"))
                // }
                const data = value;
    ...
    async getAnalytics(req: IRequestType, res: Response, next: NextFunction) {
        try {
            const result = await TodoService.getAnalytics()
